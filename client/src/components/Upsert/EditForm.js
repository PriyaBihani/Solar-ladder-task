...
const EditForm = ({ data, setData }) => {
    ...
    const [stockQuantity, setstockQuantity] = useState(data && data.stockQuantity)
    const [unit, setUnit] = useState(data && data.unit)
    const [date, setDate] = useState(data && data.asOfDate)
    const [stockWarning, setStockWarning] = useState(data && data.lowQuantity == 0 ? false : true)
    const [lowQuantity, setLowQuantity] = useState(data && data.lowQuantity)
    const [purchasePrice, setPurchasePrice] = useState(data && data.purchasePrice)
    const [gst, setGst] = useState(data && data.gst)
    const [inclusive, setInclusive] = useState(data && data.inclusive ? true : false)
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        console.log({ images, itemName, category, itemCode, description, stockQuantity, unit, date, stockWarning, lowQuantity, purchasePrice, gst, inclusive })
        // if (!name)
        ...
        if (isValidaed) try {
            const res = await axios.post(
                ...
                { images, itemName, category, itemCode, description, stockQuantity, unit, asOfDate: date, stockWarning, lowQuantity, purchasePrice, gst, inclusive },
                {
                    headers: {
            ...
            console.log(res);
            setData({ images, itemName, category, itemCode, description, stockQuantity, unit, date, stockWarning, lowQuantity, purchasePrice, gst, inclusive })
            if (res.status == 200) {
            ...
            <Grid container spacing={3}>
                <Grid item sm={12} md={6}>
                    ...
                    <Paper className={classes.paper}>
                        <FormControl margin="normal" fullWidth="true">
                            <ImageUploader currentImages={data.images} setImages={setImages} />
                        </FormControl>

                        <FormControl margin="normal" fullWidth="true">
                            <TextField value={itemName} error={errors.includes("itemName")} onChange={(e) => { setitemName(e.target.value) }} size="small" id="item-name" label="Item Name" variant="outlined" />
                        </FormControl>
                        ...
                        <FormControl margin="normal" fullWidth="true">
                            <Grid container>
                                <TextField value={itemCode} type="number" error={errors.includes("itemCode")} onChange={(e) => { setitemCode(e.target.value) }} size="small" id="item-code" label="ItemCode" variant="outlined" />
                            </Grid>
                        </FormControl>
                        <FormControl margin="normal" fullWidth="true">
                            <TextField value={description} error={errors.includes("description")} onChange={(e) => { setDescription(e.target.value) }} size="small" id="item-description" label="Description" variant="outlined" />
                        </FormControl>
                    </Paper>
                </Grid>
                ...
                <Grid item sm={12} md={6}>
                    ...
                    <Grid container>
                        ...
                        <Grid item xs={12} sm={6}>
                            <FormControl margin="normal" size="small" variant="outlined" className={classes.formControl}>
                                <TextField value={stockQuantity} error={errors.includes("stockQuantity")} onChange={(e) => { setstockQuantity(e.target.value) }} type="number" size="small" id="item-name" label="Opening Stock" variant="outlined" InputProps={{
                                    endAdornment: (
                    ...
                    <FormControl margin="normal">
                        <TextField onChange={e => { setDate(e.target.value) }} focused value={date} label="As of date" type="date" size="small" id="item-name" variant="outlined" />
                    </FormControl>
                    <br />
                    ...
                    <FormGroup row>
                        ...
                        {stockWarning ? (<FormControl margin="normal">
                            <TextField InputProps={{
                                inputProps: { min: 5 }
                            }} value={lowQuantity} onChange={e => { setLowQuantity(e.target.value) }} label="Stock Warning at" type="number" size="small" id="purchasePrice" variant="outlined" />
                        </FormControl>) : null}
                    </FormGroup>
                    ...
                    <FormGroup row>
                        <FormControl margin="normal">
                            <TextField value={purchasePrice} InputProps={{
                                inputProps: { min: 0 }
